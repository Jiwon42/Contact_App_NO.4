...
import ContactInfo from './ContactInfo4';
import ContactDetail4 from './ContactDetail4';
import './style.css'

class Contact4 extends Component {
  state = {
    keyword : '',     //입력된 검색어를 의미하는 state
    selectedKey : -1,  //selectedKey 를 -1로 초기화
                       //(ContactInfo 컴포넌트를 만든 배열의 index 값을 사용할 예정이므로 0부터 시작
                       //아무것도 선택되어 있지 않은 상태로 초기화 하려면 -1.
    contactData : [{
      name : '정지원',
      phone : '010-1234-5678'
    }, {
  ...
  _changeContact = (e) => { // 2. input 태그의 값이 변경 될 때마다 this.state.keyword 값이 변경
    this.setState({
      keyword : e.target.value
    });
  }

  _nameClick = (key) => { // 2. input 태그의 값이 변경 될 때마다 this.state.keyword 값이 변경
    this.setState({
      selectedKey : key
    });
  }
      ...
      return data.map( // 5. 해당 data 배열을 매핑
        (contact, i) => { // map 메서드의 첫 번째 인자 - item, 두 번째 인자 - index
          return (<ContactInfo 
                ...
                />); // map 메서드의 리턴값은 콜백함수의 리턴값을 각각의 원소로하는 기존배열과 같은 길이의 배열
    ...
    return(
      <div className='container'>
        ...
        <input
          ...
          onChange={this._changeContact} // 1. input 창에 입력될 때 마다
        />
